...
import { buildPrompt, buildReviewPrompt } from './prompt'
import { createIssue, commentOnIssue, createPR, linkPRToIssue, getPRInfo, commentOnPR } from './clients/github'
...
function formatAnalysisComment(analysis: string, confidence: string, hasChanges: boolean): string {
  let comment = `## 🤖 oncall-agent Analysis
 ...
 */
async function handleReviewMode(inputs: ActionInputs): Promise<ActionResult> {
  if (!inputs.prNumber) {
  ...
  // Get PR info
  const prInfo = await getPRInfo(octokit, inputs.prNumber)
  ...
  await checkoutPRBranch(inputs.prNumber)
  ...
  if (claudeResult.hasChanges) {
    ...
    // Commit
    const committed = await commitReviewChanges(inputs.commentBody)
    if (committed) {
      // Push to PR branch
      await pushToPRBranch(inputs.prNumber, prInfo.headRef)
...
async function run(): Promise<void> {
  ...
  try {
    ...
    const prompt = buildPrompt(alert, config, inputs.testCommand)
